...
public class RectangularGridTest {
	...
	/**
	 * Create a small RectangularGrid to be used for testing. The grid is 3 cells in
	 * length and 4 cells in width.
	 */
	@Before
	...
	@Test(expected = IllegalArgumentException.class)
	public void invalidConstructionX() {
		new RectangularGrid<GameObject>(0, 1);
	}

	@Test(expected = IllegalArgumentException.class)
	public void invalidConstructionY() {
		new RectangularGrid<GameObject>(1, 0);
	}
	...
	@Test(expected = IndexOutOfBoundsException.class)
	public void invalidGridAccessNegative() {
		grid.get(-1, 3);
	}
	...
	public void validGridAccess() {
		...
		assertThat("Item retrieved does not match item inserted at the same position", itemToInsert,
				is(equalTo(itemRetrieved)));
	}
}
